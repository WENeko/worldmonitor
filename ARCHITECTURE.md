...
- **Fail-closed classification**: `mode` must be `PAPER` / `PAPER_SYNTHETIC_TEST` / `RESEARCH` (else `REJECTED`); `NO_ACTION` / `PAUSE_TRADING` / `DE_RISK` are recorded as `NO_EXECUTION`, never executed (DE_RISK position reduction is a later version); actionable directives (`INCREASE_LONG_*`) are executed with hard prompt guardrails — paper only, connector `alpaca-paper-trade`, max size, no leverage/options. Idempotent by `directive_id` (a `GATED` receipt is not final). `BRIDGE_DRY_RUN=1` and a `halt` file give cheap kill switches.
- **Research commissions** (`mode: RESEARCH`): never executed against the broker — the agent answers a bounded `research_question` read-only (quotes, bars, indicators, evidence). Gated by `BRIDGE_ALLOW_RESEARCH`, default **off**: a closed gate parks the commission as a `GATED` receipt that is re-processed automatically once enabled. This is the mechanical half of Hermès's `commission_research` skill — indicator math stays in Vibe-Trading, never in the Macro Director (see `hermes-contract-v4.md`, the maintained INTERFACE CONTRACT that adds the `read_receipts` + `commission_research` + single-interface rules to Hermès's persisted file).
...
| `BRIDGE_POLL_S` / `BRIDGE_MAX_ITER` / `BRIDGE_TIMEOUT_S` / `BRIDGE_DRY_RUN` / `BRIDGE_ALLOW_RESEARCH` | `deploy/oci/.env` (optional) | directive-bridge tuning: watch tick, agent iteration cap, agent timeout, dry-run (log prompt, never invoke the agent), research-commission gate (default off) |
