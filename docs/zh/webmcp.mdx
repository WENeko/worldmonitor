...
六个仪表板变体都注册相同的二十四个命令式工具。登录和权益变化不会改变注册集合；每次调用都会重新检查实时状态。

在没有提供目标侧调用信号的浏览器上，可逆的视图状态工具仍会执行。此类浏览器无法取消这些工具，所以中止调用后，可见的仪表板变化仍可能生效。`openCountryBrief`、`switch_monitor`、`set_panel_enabled`、`set_map_layers`、`set_map_mode`、`select_dashboard_tab`、`create_dashboard_tab`、`rename_dashboard_tab` 与 `delete_dashboard_tab` 会返回不可用。`open_search_result` 仍会执行视图状态结果，例如打开已启用的实时摄像头面板，并拒绝持久化、消耗配额或外部导航结果。国家简报工具可能在调用方取消后继续消耗每日 LLM 额度。监视器切换可能持久化并重新加载或导航到其他源。面板、图层、模式和标签页变更会在本次会话之外持久化。只读的 `list_map_layers` 和 `list_dashboard_tabs` 目录仍会执行。参见[可用性](#availability)。
