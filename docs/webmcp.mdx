...
Every dashboard variant registers the same twenty-four imperative tools. Registration is stable across sign-in and entitlement changes; each invocation rechecks the live state.

On a browser that omits the target-side invocation signal, the reversible view-state tools still run. Such a browser cannot cancel them, so an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `switch_monitor`, `set_panel_enabled`, `set_map_layers`, `set_map_mode`, `select_dashboard_tab`, `create_dashboard_tab`, `rename_dashboard_tab`, and `delete_dashboard_tab` reject as unavailable there instead. `open_search_result` still runs a view-state result, such as opening an already-enabled live-webcam panel, and rejects a persistent, quota-consuming, or external-navigation result. The country tool can consume daily LLM allowance after caller cancellation. Monitor switching can persist and reload or navigate to another origin. Panel, layer, mode, and tab writes persist beyond the session. The read-only `list_map_layers` and `list_dashboard_tabs` catalogs still run. See [Availability](#availability).
